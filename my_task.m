...
for iter = 1:maxiter
  ...
  for mesh = minmesh:maxmesh 
    ...
    q   = y.*sqrt(abs(4*dydx + y.^4)) + 2*y.*dydx./sqrt(abs(4*dydx + y.^4)) - y.^3;
    p   = dydx.*sqrt(abs(4*dydx + y.^4)) + 2*y.^4.*dydx./sqrt(abs(4*dydx + y.^4)) - 3*y.^2.*dydx;
    phi = y.*dydx.*sqrt(abs(4*dydx + y.^4))-y.^3.*dydx-d2ydx2;
    ...
    b(M)   = 1;
    c(M)   = 0;
    phi(M) = YR - phi(M - 1)/(2*h*c(M - 1)) - dydx(M) * y(m);
